...
public class QuestionSendActivity extends AppCompatActivity implements View.OnClickListener, DatabaseReference.CompletionListener {
    ...
    public void onClick(View v) {
        if (v == mImageView) {
            ...
            DatabaseReference pushedReference;

            pushedReference = genreRef.push();
            pushedReference.setValue(data, this);
            //pushされたデータ直下にお気に入りリストを登録しておく
            ArrayList<String> arrayList = new ArrayList<>();
            //空のリストを登録できないので、空である要素を一つ追加する
            arrayList.add("");

            pushedReference.child("favoriteUserList").setValue(arrayList, this);
            mProgress.show();
        }
    }
